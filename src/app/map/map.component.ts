...
import {divIcon, icon, latLng, map, Map, marker, point, polyline, tileLayer} from 'leaflet';
declare const $: any;
...
export class MapComponent implements OnInit {
  ...
  ngOnInit() {
    $( document ).ready(function($) {
      $('#select2-driver-list1').select2({
      });
      $('#select2-driver-list3').select2({
      });
      $('#select2-driver-list2').select2({
      });
      $('.date-driver-list1').persianDatepicker({
        initialValueType: 'persian',
        // initialValue: false
      });
      $('.date-driver-list2').persianDatepicker({
        initialValueType: 'persian',
        // initialValue: false
      });
    });
